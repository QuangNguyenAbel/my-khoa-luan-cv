                ...
                if ($db->num_rows($sql)) {
                    ...
                    foreach ($db->fetch_assoc($sql, 0) as $key => $row) {
						...
						<div class="form-group">
							<label> Tựa Đề </label>
							<input type="text" name="tua_de"  readonly value="' . $row['tua_de'] . '" class="form-control">
						</div>
                        <div class="form-group">
							<label> Nội Dung </label>
                            <textarea class="form-control" readonly id="editor1" name="nd" rows="4" cols="50" >' . $row['noi_dung'] . '</textarea>
						</div>
                        <div class="form-group">
						...
						<a href="' . $_DOMAIN . 'thongbao_noibo" class="btn btn-info"> Quay Lại </a>
					</form>';
                    }
                }
            }
